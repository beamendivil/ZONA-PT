...
import { ArrowRight, Smile, Moon, Briefcase } from 'lucide-react';

gsap.registerPlugin(ScrollTrigger);

const outcomes = [
  { icon: Smile, text: 'Less pain in daily tasks' },
  { icon: Moon, text: 'Better sleep & energy' },
  { icon: Briefcase, text: 'Return to sport or work' },
];
      ...
      <div className="absolute inset-0 flex items-center px-[7vw]">
        ...
        <div ref={headlineRef} className="w-[42vw] max-w-[500px]">
          <h2 className="font-heading font-extrabold text-[clamp(36px,5vw,72px)] text-[#1A2D3D] leading-[0.95] tracking-[0.02em] uppercase">
            <span className="accent-underline">RECOVERY</span>
          </h2>
          <p className="mt-4 text-[clamp(14px,1.1vw,18px)] text-[#1A2D3D] leading-relaxed">
            Feel the difference in weeks, not months—with clear milestones along the way.
          </p>
        </div>
              ...
              {outcomes.map((outcome, index) => (
                <li key={index} className="flex items-center gap-4">
                  <div className="w-10 h-10 rounded-full bg-[#EAF4FA] flex items-center justify-center flex-shrink-0">
                    <outcome.icon className="w-5 h-5 text-[#2F9BFF]" />
                  </div>
                  <span className="text-[#1A2D3D] font-medium">{outcome.text}</span>
                </li>
              ))}
            </ul>
